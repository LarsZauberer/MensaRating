...
BLACKLIST = ["Ferien", "Kollegiumstag"]
...
def get_menu_list(days):
    ...
    for menuIndex, day in enumerate(days):
        ...
        for dishIndex, dish in enumerate(dishes):
            ...
            if (
                dish.find(attrs={"class": "label label-vegetarian has-infobox"})
                is not None
            ):
                label = 1
            elif (
                dish.find(attrs={"class": "label label-vegan has-infobox"}) is not None
            ):
                label = 2
            else:
                label = 0
...
def clean(str):
    ...
    string = re.sub(
        '<p class="menu-description">|</p>|<h2 class="menu-title">|</h2>|\\xad\s*|\\n|amp;',
        "",
        str,
    )
...
def get_menu_type(title: str) -> MenuType:
    ...
    if len(menuType) == 0:
        ...
        try:
            ...
            log.critical(
                f"Debug information: title: {title}, description: {description}, label: {label}, date: {date}, menuType: {menuType}"
            )
            return
    ...
    for key, date in zip(data.keys(), dates):
        ...
        for index, item in enumerate(data[key].keys()):
            ...
            try:
                menu = menus[index]
                log.debug(
                    f"Menu \"{data[key][item]['title']}\" already in database (DB: {menu})"
                )
                update_menu_in_database(
                    data[key][item]["title"],
