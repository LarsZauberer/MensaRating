...
Die Menus werden von der Mensa-Website \url{https://neuekanti.sv-restaurant.ch/}
...
\code{Menu}-Objekt (siehe \ref{spez:Menus}) gibt. Wenn nicht, dann wird ein
neues \code{Menu}-Objekt erstellt und dazu, wenn auch dieses nicht existiert ein
neues \code{MenuType}-Objekt (siehe \ref{code:core.models.py}). Wenn das
\code{Menu}-Objekt bereits existiert hat, werden die Daten einfach ersetzt und
es wird gegebenenfalls ein neues \code{MenuType}-Objekt erstellt. Wenn die
Synchronization durch ist, werden \code{MenuType}-Objekte gelöscht, welche keine
zugehörigen \code{Menu}-Objekte haben. Das kann passieren, da durch das Updaten
der Menus ein altes \code{MenuType}-Objekt existiert, nicht nach der Mensa nie
existieren sollte.
...
Posts bekommen. (Funktion \code{add\_karma\_for\_posting}). Mit den Punkten kann
